...
logger.add(os.path.join(ProjectDir,'Logs/RNN.log'),format="{time} {level} {message}",level="INFO",rotation="200 MB",diagnose=True)
...
class NeuralNetwork:
    ...
    def BackwardPropagation(self,Input,Target):
        try:
            ...
            # Производная HiddenLayer_1
            d_hl_1 = d_h2 @ self.w2.T
            d_h1 = d_hl_1 * self.deriv_sigmoid(self.h1)
            # Производная w1,b1
            d_w1 = Input.T @ d_h1
